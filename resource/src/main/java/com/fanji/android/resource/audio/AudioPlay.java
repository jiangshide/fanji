...
import com.fanji.android.play.audio.AudioVisualConverter;
...
public class AudioPlay implements MediaPlayer.OnCompletionListener, MediaPlayer.OnInfoListener,
    ...
    public synchronized AudioPlay play(final String path) {
        String proxy = Play.INSTANCE.getAudioUrl(path);
        boolean isNet = NetState.Companion.getInstance().isNetworkAvailable();
        if (!TextUtils.isEmpty(proxy) && (proxy.contains("http") || proxy.contains("https")) && !isNet) {
            proxy = "";
            FJToast.txt("无网络，无缓存,暂不能播放!");
        }
    ...
    public AudioPlay setData(ArrayList<Feed> data) {
        this.mData = data;
        return this;
    }

    public int setData(Feed data) {
        if (mData == null) {
            mData = new ArrayList<>();
        }
    ...
    public AudioPlay update(Feed data) {
        int count = count();
    ...
    public AudioPlay play(Feed blog) {
        if (mData != null && mData.size() > 0) {
