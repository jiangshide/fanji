package com.fanji.android.resource.vm.publish.data
...
data class File(
  ...
  override fun describeContents(): Int {
    ...
    override fun toString(): String {
        return "File(id=$id, contentId=$contentId, classify=$classify, url=$url, cover=$cover, dynamicCover=$dynamicCover, name=$name, sufix=$sufix, format=$format, duration=$duration, width=$width, height=$height, size=$size, rotate=$rotate, bitrate=$bitrate, sampleRate=$sampleRate, level=$level, mode=$mode, wave=$wave, lrcZh=$lrcZh, lrcEs=$lrcEs, source=$source, randomH=$randomH)"
    }
