...
import com.fanji.android.util.StringUtil
...
//        text.setDrawableLeft(R.mipmap.sex_women)
  ...
  fun setIcon(
    ...
    if (isCircle) {
      FJImg.loadImageCircle(url, img)
    } else {
      FJImg.loadImageRound(url, img, 5)
    }
  }
  ...
  fun setCover(activity: Activity, img:ImageView,isRandom: Boolean){
    ...
    FJImg.loadImageRound(cover, img, 10,R.mipmap.splash)
  }
  ...
  override fun toString(): String {
    return "Channel(id=$id, uid=$uid, online=$online, onlineId=$onlineId, onlineName=$onlineName, typeId=$typeId, natureId=$natureId, cover=$cover, name=$name, des=$des, status=$status, official=$official, reason=$reason, date=$date, icon=$icon, nick=$nick, sex=$sex, age=$age, city=$city, latitude=$latitude, longitude=$longitude, locationType=$locationType, blogNum=$blogNum, url=$url, remark=$remark, tops=$tops, follows=$follows, reportr=$reportr, index=$index, refresh=$refresh)"
  }
