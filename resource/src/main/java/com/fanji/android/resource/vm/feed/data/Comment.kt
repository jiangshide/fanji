...
class Comment() : Parcelable {
  ...
  var at:String?=""//@内容:content:id
  var topic:String?=""//频道(话题)内容:content:id
  ...
  override fun toString(): String {
    return "Comment(id=$id, uid=$uid, fromUid=$fromUid, contentId=$contentId, content=$content, urls=$urls, reply=$reply, replyNick=$replyNick, status=$status, reason=$reason, date=$date, nick=$nick, icon=$icon, remark=$remark, num=$num, praiseNum=$praiseNum, praises=$praises, reportr=$reportr, comments=$comments)"
  }
