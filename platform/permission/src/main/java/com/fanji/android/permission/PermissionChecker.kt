...
internal object PermissionChecker {
    ...
    fun checkPermissionArgument(requestPermissions: List<String>?, debugMode: Boolean): Boolean {
        ...
        if (debugMode) {
            ...
            for (permission in requestPermissions) {
                require(allPermissions.contains(permission)) {
                    // 请不要申请危险权限和特殊权限之外的权限
                    "The $permission is not a dangerous permission or special permission"
                }
            }
        }
        return true
    ...
    fun checkPermissionManifest(context: Context, requestPermissions: List<String>) {
        val manifestPermissions: List<String>? = PermissionUtils.getManifestPermissions(context)
