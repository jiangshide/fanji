 ...
 * @since 21-5-26 上午11:59
 */
internal class PermissionFragment : Fragment(), Runnable {
    /** 是否申请了特殊权限  */
    ...
    override fun onAttach(context: Context) {
        ...
        val activityOrientation: Int = activity.resources.configuration.orientation
        try {
    ...
    fun requestSpecialPermission() {
        ...
        if (PermissionUtils.containsSpecialPermission(permissions)) {
                ...
                if (PermissionUtils.isAndroid11()) {
                    ...
                    getArguments()?.let {
                        startActivityForResult(
                            PermissionSettingPage.getStoragePermissionIntent(activity),
                            it.getInt(
                                REQUEST_CODE
                            )
                        )
            ...
            if (permissions.contains(Permission.REQUEST_INSTALL_PACKAGES) && !PermissionUtils.isGrantedInstallPermission(
                ...
                getArguments()?.let {
                    startActivityForResult(
                        PermissionSettingPage.getInstallPermissionIntent(activity),
                        it.getInt(
                            REQUEST_CODE
                        )
                    )
            ...
            if (permissions.contains(Permission.SYSTEM_ALERT_WINDOW) && !PermissionUtils.isGrantedWindowPermission(
                ...
                getArguments()?.let {
                    startActivityForResult(
                        PermissionSettingPage.getWindowPermissionIntent(activity),
                        it.getInt(
                            REQUEST_CODE
                        )
                    )
            ...
            if (permissions.contains(Permission.NOTIFICATION_SERVICE) && !PermissionUtils.isGrantedNotifyPermission(
                ...
                getArguments()?.let {
                    startActivityForResult(
                        PermissionSettingPage.getNotifyPermissionIntent(activity),
                        it.getInt(
                            REQUEST_CODE
                        )
                    )
            ...
            if (permissions.contains(Permission.WRITE_SETTINGS) && !PermissionUtils.isGrantedSettingPermission(
                ...
                getArguments()?.let {
                    startActivityForResult(
                        PermissionSettingPage.getSettingPermissionIntent(activity),
                        it.getInt(
                            REQUEST_CODE
                        )
                    )
                    ...
                    false, object : OnPermissionCallback {
                        ...
                        override fun onDenied(permissions: List<String?>?, never: Boolean) {
                            ...
                            for (i in allPermissions.indices) {
                                grantResults[i] = if (Permission.ACCESS_BACKGROUND_LOCATION == allPermissions[i]
    ...
    override fun onRequestPermissionsResult(
        ...
        for (i in permissions.indices) {
            ...
            if (!PermissionUtils.isAndroid10() &&
                (Permission.ACCESS_BACKGROUND_LOCATION == permission ||
                        Permission.ACTIVITY_RECOGNITION == permission ||
            ...
            if (!PermissionUtils.isAndroid9() &&
                Permission.ACCEPT_HANDOVER == permission
            ) {
                // 如果当前版本不符合最低要求，那么就重新进行权限检测
                grantResults[i] = PermissionUtils.getPermissionStatus(activity, permission)
                continue
            ...
            if (!PermissionUtils.isAndroid8() &&
                (Permission.ANSWER_PHONE_CALLS == permission ||
        ...
        if (grantedPermission.size == permissions.size) {
            if (useInterceptor) {
                // 代表申请的所有的权限都授予了
                FJPermission.interceptor
                    ?.grantedPermissions(activity, callback, grantedPermission, true)
            } else {
        ...
        val deniedPermission: List<String?> =
            PermissionUtils.getDeniedPermissions(permissions, grantResults)
        if (useInterceptor) {
            // 代表申请的权限中有不同意授予的，如果有某个权限被永久拒绝就返回 true 给开发人员，让开发者引导用户去设置界面开启权限
            FJPermission.interceptor?.deniedPermissions(
                activity,
                callback,
                deniedPermission,
        ...
        if (!grantedPermission.isEmpty()) {
            if (useInterceptor) {
                FJPermission.interceptor
                    ?.grantedPermissions(activity, callback, grantedPermission, false)
            } else {
