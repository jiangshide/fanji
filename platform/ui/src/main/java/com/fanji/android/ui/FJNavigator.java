...
import com.fanji.android.ui.tablayout.help.NavigatorHelper;
...
import com.fanji.android.ui.tablayout.interfaces.IScrollState;
 ...
 */
public class FJNavigator extends FrameLayout implements IPagerNavigator, NavigatorHelper.OnNavigatorScrollListener{
    ...
    private List<TabPosition> mPositionDataList = new ArrayList<TabPosition>();
    ...
    public FJNavigator(Context context) {
        super(context);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mAdapter != null) {
            ...
            if (mReselectWhenLayout && mNavigatorHelper.getScrollState() == IScrollState.SCROLL_STATE_IDLE) {
    ...
    private void preparePositionData() {
        ...
        for (int i = 0, j = mNavigatorHelper.getTotalCount(); i < j; i++) {
            TabPosition data = new TabPosition();
        ...
        if (mAdapter != null) {
            ...
            if (mScrollView != null && mPositionDataList.size() > 0 && position >= 0 && position < mPositionDataList.size()) {
                if (mFollowTouch) {
                    ...
                    TabPosition current = mPositionDataList.get(currentPosition);
                    TabPosition next = mPositionDataList.get(nextPosition);
    ...
    public void onSelected(int index, int totalCount) {
        ...
        if (!mAdjustMode && !mFollowTouch && mScrollView != null && mPositionDataList.size() > 0) {
            ...
            TabPosition current = mPositionDataList.get(currentIndex);
