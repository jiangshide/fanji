...
import com.fanji.android.ui.spedit.gif.span.GifIsoheightImageSpan
import java.io.File
import java.util.*
...
object EmojiManager {
    ...
    fun init(context: Context) {
        val unziped = EmojiUtil.getEmojiInitResult(context)
        if (unziped) {
            initDefault(context)
        } else {
            Thread(Runnable {
                val unzipResult =
                    EmojiUtil.unzipFromAssets(context, EmojiUtil.getEmojiDir(context), "emoji")
                EmojiUtil.setEmojiInitResult(context, unzipResult)
                if (unzipResult) {
                    initDefault(context)
                }
            }).start()
