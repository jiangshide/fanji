...
class EmojiconScrollTabBar @JvmOverloads constructor(
    ...
    private var tab_container: LinearLayout
    private var scroll_view: HorizontalScrollView

    init {
        val view = LayoutInflater.from(mContext)
            .inflate(R.layout.common_emoj_widget_emojicon_tab_bar, this)
    ...
    fun addTab(icon: Int) {
        ...
        val imgParams = LinearLayout.LayoutParams(
            EmojiUtil.dp2px(mContext, tabWidth), LayoutParams.MATCH_PARENT
        )
    ...
    fun selectedTo(position: Int) {
        ...
        for (i in tabList.indices) {
            if (position == i) {
                tabList[i].setBackgroundColor(
                    ContextCompat.getColor(
                        mContext,
                        R.color.emojicon_tab_selected
                    )
                )
            } else {
                tabList[i].setBackgroundColor(
