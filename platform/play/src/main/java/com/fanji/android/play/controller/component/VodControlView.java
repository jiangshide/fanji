package com.fanji.android.play.controller.component;
...
import com.fanji.android.play.dplay.controller.ControlWrapper;
import com.fanji.android.play.dplay.controller.IControlComponent;
...
import static com.fanji.android.play.dplay.util.PlayerUtils.stringForTime;
...
public class VodControlView extends FrameLayout implements IControlComponent, View.OnClickListener, SeekBar.OnSeekBarChangeListener {
    ...
    
    {
        ...
        mSoundState = findViewById(R.id.soundState);
        mSoundState.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if(mControlWrapper == null) return;
                boolean sound = !SPUtil.getBoolean(SOUND,false);
                SPUtil.putBoolean(SOUND,sound);
                mSoundState.setSelected(sound);
                mControlWrapper.setMute(sound);
            }
        });
    ...
    public void onVisibilityChanged(boolean isVisible, Animation anim) {
        if (isVisible) {
            ...
            setSound();
            mSoundState.setVisibility(VISIBLE);
            if (anim != null) {
                mBottomContainer.startAnimation(anim);
                mSoundState.startAnimation(anim);
            }
            ...
            mSoundState.setVisibility(GONE);
            if (anim != null) {
                mBottomContainer.startAnimation(anim);
                mSoundState.startAnimation(anim);
            }
    ...
    private void setSound(){
        if(mControlWrapper == null || mSoundState == null) return;
        if( mControlWrapper.isFullScreen()){
            mControlWrapper.setMute(false);
            mSoundState.setSelected(false);
        }else{
            boolean isSound = SPUtil.getBoolean(SOUND,false);
            mSoundState.setSelected(isSound);
            mControlWrapper.setMute(isSound);
        }
    }
    ...
    private void toggleFullScreen() {
        Activity activity = PlayerUtils.scanForActivity(getContext());
        mControlWrapper.toggleFullScreenByVideoSize(activity);//横竖屏切换，根据适配宽高决定是否旋转屏幕
    }
