...
class RealConnectionPool(
  ...
  /**
   * close idle connections with no idle connections no accept stream
   */
  fun forceEvictAll() {
    val i = connections.iterator()
    while (i.hasNext()) {
  ...
  /**
   * close idle connections by host with no idle connections no accept stream
   */
  fun forceEvictByHost(host: String?) {
    val i = connections.iterator()
    while (i.hasNext()) {
      val connection = i.next()
      if (host != connection.route().address.url.host) continue
      val socketToClose = synchronized(connection) {
        connection.noNewExchanges = true
        if (connection.calls.isEmpty()) {
          i.remove()
